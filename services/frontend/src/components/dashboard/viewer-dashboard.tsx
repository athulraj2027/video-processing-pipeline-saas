...
import { 
  Compass, PlayCircle, Star, Film, CreditCard, Play, X, RefreshCw, Check, Sparkles 
} from 'lucide-react';
...
export function ViewerDashboard() {
  ...
  const [rentals, setRentals] = useState<string[]>(['film-1']); // film-1 starts unlocked
    ...
    <div className="flex-1 flex flex-col md:flex-row">
      ...
      <nav className="w-full md:w-64 border-r border-border bg-muted/10 p-4 shrink-0 flex flex-row md:flex-col gap-2 md:gap-1.5 overflow-x-auto md:overflow-x-visible">
        ...
        >
          ...
          <span>My Library ({rentals.length})</span>
        </button>
      </nav>

      {/* Viewer Workstation */}
      <main className="flex-1 p-6 md:p-8 overflow-y-auto max-w-7xl mx-auto w-full">
        {/* BROWSE STOREFRONT TAB */}
        {activeTab === 'browse' && (
                  ...
                  return (
                    <Card key={film.id} className="border-border bg-card shadow-sm flex flex-col justify-between overflow-hidden group">
                      <div className="p-5 space-y-4">
                        <div className="h-40 bg-muted/40 rounded-xl flex items-center justify-center text-muted-foreground group-hover:text-emerald-500 transition-colors relative animate-fade-in">
        ...
        {activeTab === 'library' && (
          <div className="space-y-6">
            ...
            {films.filter(f => rentals.includes(f.id)).length === 0 ? (
              <Card className="p-8 text-center border-dashed border-2 border-border flex flex-col items-center">
                ...
                <Button 
                  onClick={() => setActiveTab('browse')}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg flex items-center gap-1 text-xs font-semibold"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {films.filter(f => rentals.includes(f.id)).map((film) => (
                  <Card key={film.id} className="border-border bg-card shadow-sm flex flex-col justify-between overflow-hidden group">
                    <div className="p-5 space-y-4">
      ...
      {showPlayerModal && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-4xl space-y-4 relative animate-fade-in">
            <button 
