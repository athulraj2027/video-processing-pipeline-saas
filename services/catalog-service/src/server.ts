...
async function start() {
  try {
    ...
    registerCleanupTask(async () => {
      try {
        console.log('🔌 Disconnecting Prisma database client for catalog-service...');
    ...
    createAndStartServer(app, env.PORT, env.HOST, () => {
      console.log(`🚀 Catalog Service running at http://${env.HOST}:${env.PORT}`);
      console.log(`📡 Base endpoint: http://${env.HOST}:${env.PORT}/api/v1/catalog`);
    });
