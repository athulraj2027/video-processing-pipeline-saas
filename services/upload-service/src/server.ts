...
async function start() {
  try {
    ...
    registerCleanupTask(async () => {
      try {
        console.log('🔌 Disconnecting Prisma database client for upload-service...');
    ...
    createAndStartServer(app, env.PORT, env.HOST, () => {
      console.log(`🚀 Upload Service running at http://${env.HOST}:${env.PORT}`);
      console.log(`📡 Base endpoint: http://${env.HOST}:${env.PORT}/api/v1/upload`);
    });
